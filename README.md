...
이 프로젝트는 네트워크 운영 흐름을 학습하기 위한 개인 시뮬레이션 프로젝트이므로,
실제 데이터센터 환경의 장비, 프로토콜, 트래픽 규모를 그대로 재현하지는 못했습니다.

현재 실제 측정 영역은 Windows PC에서 수행한 ICMP Ping의 RTT와 Packet Loss이며,
Topology, Traffic, Device 상태, 장애 발생과 대응 과정은 가상 데이터와 규칙을 기반으로 구성했습니다.

향후에는 다음 내용을 추가해 프로젝트의 현실성을 높이고자 합니다.

- OSPF, BGP 등 Routing Protocol에 따른 경로 변화 반영
- VLAN과 Subnet을 활용한 네트워크 구간 분리
- Firewall, VPN, Load Balancer 등 인접 인프라 요소 연동
- 실제 장비 Telemetry와 유사한 Interface Error, Drop, Routing 상태 추가
- 서비스별 SLA 기준을 반영한 품질 판단 고도화
- 반복 장애 이력을 활용한 Event 및 Incident 분석 개선

## 11. What I Learned

프로젝트를 진행하며 네트워크 운영은 단순히 장애가 발생한 장비를 복구하는 업무가 아니라,
평상시 상태를 관찰하고 이상 징후를 발견한 뒤 영향 범위와 원인 후보를 좁혀 대응하고,
대응 이후 서비스 품질이 정상화됐는지 다시 확인하는 과정이라는 점을 배웠습니다.

또한 같은 장애라도 서비스의 특성과 영향을 받는 구간에 따라
우선적으로 확인해야 할 Metric과 대응 기준이 달라질 수 있다고 판단했습니다.

Python과 생성형 AI를 활용해 코드를 구현하는 과정에서도
기능 자체보다 어떤 기준으로 상태를 판단하고,
왜 해당 대응과 검증 절차가 필요한지를 직접 정의하는 데 중점을 두었습니다.
